import java.lang.reflect.*;
public class Main {
  public static void main(String[] args) {
    //Uncomment lines below to test Problem 1 (Shape.java)
    /*
    Shape triangle = new Shape(3);
    Shape square = new Shape(4);
    Shape hexagon = new Shape(6);
    Shape hecatontagon = new Shape(100);

    //Tests if your instance variables are private or not
    assert Shape.class.getFields().length == 0: "Test case failed: one or more instance variables are not private";

    //Tests calculateAngle()
    assert triangle.calculateAngle() == 60: "calculateAngle() failed for sides = 3";

    assert square.calculateAngle() == 90: "calculateAngle() failed for sides = 4";

    assert hexagon.calculateAngle() == 120: "calculateAngle() failed for sides = 6";

    assert hecatontagon.calculateAngle() == 176.4: "calculateAngle() failed for sides = 100";

    //Runs main - have mentor check this one
    Shape.main(args);
    */

    //To test Bonus Problem, ask a mentor to run the examples above with you (i.e., enter 3, 4, 6, 7, 100 and so on).
    //BonusProblem.getAngle();
  }
}
